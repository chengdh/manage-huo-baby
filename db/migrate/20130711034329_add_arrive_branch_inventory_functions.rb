#coding: utf-8
#添加分理处盘货单货场确认功能
class AddArriveBranchInventoryFunctions < ActiveRecord::Migration
  def  change
    ##############################到货清单管理#############################################
    group_name = "盘货操作"
    subject_title = "分理处盘货单(待确认)"
    subject = "BranchInventory"
    sf_hash = {
      :group_name => group_name,
      :subject_title => subject_title,
      :default_action => 'arrive_branch_inventories_path("search[state_eq]" => "shipped","search[to_org_id_in]" => current_user.current_ability_org_ids)',
      :subject => subject,
      :function => {
