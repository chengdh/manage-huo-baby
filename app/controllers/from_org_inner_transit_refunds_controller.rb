#coding: utf-8
class FromOrgInnerTransitRefundsController < BaseRefundsController
  ...
  protected
  def collection
    @search = end_of_association_chain.accessible_by(current_ability,:read_from_org).from_org_refunds(current_user.current_ability_org_ids).search(params[:search])
